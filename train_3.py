from myModelFactory_3 import ModelFactory
from callback_3 import MultipleClassAUROC, MultiGPUModelCheckpoint
import pandas as pd
import numpy as np
...
def augmentation(X,Y):
    ...
    datagen.fit(X)
    a,b = next( datagen.flow(X, Y, batch_size= X.shape[0], seed = int( time.time() ) ) )
    gen_data = []
    seq = iaa.Sequential([iaa.CoarseDropout(p=0.05,size_percent=0.2)])
...
#modelcheckpoint = ModelCheckpoint('./model/weights.{epoch:03d}-{val_loss:.5f}.h5',mode='min',monitor='val_loss',save_best_only=True)
