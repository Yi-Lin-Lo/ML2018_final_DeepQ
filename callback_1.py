...
epoch_threshold = 8
mean_auroc_threshold = 0.75
loss_threshold = 0.1860

class MultipleClassAUROC(Callback):
    """
    Monitor mean AUROC and update model
    """
    def __init__(self, base_model, sequence, valid_y, class_names, weights_path, model_name, stats=None, workers=1):
        ...
        if stats:
            ...
            self.stats = {"best_loss": 100, "best_mean_auroc": 0, }
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        """
        y_hat = self.model.predict(self.sequence)
        y = self.valid_y
        #self.sequence.get_y_true()
        ...
        if loss < self.stats["best_loss"]:
            print(loss)
            if epoch > epoch_threshold and mean_auroc > mean_auroc_threshold and loss<loss_threshold:
                self.stats["best_loss"] = loss
                print(self.model_name, " saved!")
                self.base_model.save(os.path.join(os.path.split(self.weights_path)[0],self.model_name,), overwrite=True)
        return
